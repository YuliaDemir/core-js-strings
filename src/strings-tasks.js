...
function countVowels(str) {
  const vowelArray = str.match(/[aeiouyAEIOUY]/g);
...
function isPalindrome(str) {
  const normalStr = str.replaceAll(/\W/g, '');
  if (str.length < 2) return true;
  let left;
  let right;
  if (normalStr.length % 2 === 0) {
    left = normalStr.substring(0, normalStr.length / 2);
    right = normalStr.substring(normalStr.length / 2);
  } else {
    left = normalStr.substring(0, (normalStr.length - 1) / 2);
    right = normalStr.substring((normalStr.length + 1) / 2);
  }
...
function findLongestWord(sentence) {
  const array = sentence.split(/\W/);
  if (!array || array === '') return '';
  let longestWord = '';
  let i = 0;
  while (i < array.length) {
    if (array[i] && array[i].length > longestWord.length) {
      longestWord = array[i];
    }
    i += 1;
  }
...
function invertCase(str) {
  return str
    .split('')
    .map((s) => (s.match(/[a-z]/) ? s.toUpperCase() : s.toLowerCase()))
    .join('');
}
...
function extractNameFromTemplate(value) {
  const regex = /Hello, ([a-zA-Z]+) ([a-zA-Z]+)!/;
  const result = value.match(regex);
  return result ? result[1].concat(' ', result[2]) : '';
}
...
function unbracketTag(str) {
  ...
  return result[1].concat(result[2], result[3]);
}
...
function encodeToRot13(str) {
  return String.fromCharCode(
    ...str.split('').map((letter) => {
      if (letter.match(/[A-Ma-m]/)) return letter.charCodeAt(0) + 13;
      if (letter.match(/[n-zN-Z]/)) return letter.charCodeAt(0) - 13;
